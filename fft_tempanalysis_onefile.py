...
def perform_fft(data_vectors, time_vectors):
    ...
    for product_flag, data in data_vectors.items():
       ...
       # Apply Bartlett window to the data
        ...
        # #Plot Data
        plt.figure()
        plt.plot(time_vector, data, label=f'Original Data for {product_flag} GRACE-FO C: 2023-07-20')
...
filename=r'C:\data\AHK1AC\AHK1A_2023-07-20_C_04.txt' #the r with the prefix denotes the absolute path of the file C:\data\AHK1A 2022_07_01-08_31 C
...
# Averaging Filter data vectors and time vectors for each product flag (For FFT to be viable, all )
for key in data_vectors:
    averaged_data = []
    averaged_time = []
