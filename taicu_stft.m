...
taicu=readmatrix("taicu_data_avg.txt");
...
% xlabel('Time');
% ylabel('Temperature');
% title('Taicu Temperature over Time');
...
log_magnitude = log(abs(s)*2/ windowLength);
...
colorbar;
axis tight;
view(2); %Top view for better clarity
ylim([0,35])

xlabel('Date');
ylabel('Frequency (cycles/day)');
title('STFT of Taicu Signal (Log Magnitude) with Celsius Labels');

% %Checking Dimensions
% disp(t_temperature(1:5));  % Check first 5 datetime values
% disp(size(t_temperature)); % Ensure size matches number of time steps in `s`
% disp(size(f));             % Should match the number of rows in log_magnitude
% disp(size(log_magnitude)); % Check overall STFT output
...
celsius_ticks = [min(abs(s(:))/windowLength),1e-5,0.001,0.01,0.1,0.2, 1,max(abs(s(:))/windowLength)];  % Adjust based on your actual Celsius range

% Convert these Celsius values to their corresponding log scale values
log_celsius_ticks = log(celsius_ticks);

% Set the colorbar ticks to the log values
cb.Ticks = log_celsius_ticks;
...
legend('STFT Magnitude', 'Interpolated BetaPrime', 'Location', 'Best');
...
min_freq = 15.2;  % Example minimum frequency (in cycles per day)
max_freq = 15.3;  % Example maximum frequency (in cycles per day)
